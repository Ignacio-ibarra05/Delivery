...
import { crearPedido, getProductos } from "../api";
...
function MapaRestaurantesLeaflet({ restaurantes, seleccionado, onSeleccionar }) {
  ...
  // Cuando el mapa vuelve a ser visible (tras salir del menú), forzar resize
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Espera a que window.L esté disponible (puede cargarse tarde en Vite)
    function initMap() {
    ...
    restaurantes.forEach((r) => {
      ...
      // Forzar re-render del mapa por si el contenedor aún no tenía tamaño
      setTimeout(() => map.invalidateSize(), 100);
    }

    // Si window.L ya está disponible, inicializar de inmediato
    if (window.L) {
      initMap();
    } else {
      // Si no está listo, esperar con polling hasta que cargue
  ...
  // Actualizar marcadores activos + calcular ruta cuando cambia la selección
  useEffect(() => {
    const L = window.L;
    ...
    // Calcular nueva ruta si hay restaurante seleccionado y el plugin está disponible
    if (seleccionado && L.Routing) {
      const control = L.Routing.control({
        ...
        show: false,           // oculta el panel de instrucciones del plugin
      ...
      control.on("routesfound", (e) => {
        ...
        setInfoRuta({
          distancia: (ruta.totalDistance / 1000).toFixed(1), // km
          duracion: Math.ceil(ruta.totalTime / 60),          // minutos
        });
      });
  ...
  return (
    <div style={ms.mapaWrapper}>
      ...
      {/* Banner de info de ruta */}
      {seleccionado && (
        <div style={ms.rutaBanner}>
          {!infoRuta ? (
            <span style={ms.rutaCalculando}>🔄 Calculando ruta hacia <b>{seleccionado.nombre}</b>…</span>
          ) : infoRuta.error ? (
            <span style={ms.rutaError}>⚠️ No se pudo calcular la ruta. Intenta de nuevo.</span>
          ) : (
            <span style={ms.rutaInfo}>
              🗺️ Ruta a <b>{seleccionado.nombre}</b>:&nbsp;
...
function MenuRestaurante({ restaurante, onVolver }) {
  ...
  async function confirmarPedido() {
    ...
    try {
      ...
      setExito(true);
      setTimeout(() => { setExito(false); setCarrito({}); }, 4000);
            ...
            <div style={mv.menuGrid}>
              {menu.map(p => (
                <div key={p.id} style={mv.card}>
                  <div style={mv.cardEmoji}>{p.emoji || "🍽️"}</div>
        ...
        <aside style={mv.sidebar}>
          ...
          {itemsCarrito.length === 0 ? <p style={mv.muted}>Agrega productos para armar tu pedido.</p> : (
            <>
              ...
              {exito && <div style={mv.exitoBox}>✅ ¡Pedido enviado! Un repartidor lo tomará pronto.</div>}
              <button onClick={confirmarPedido} disabled={enviando || exito} style={mv.confirmBtn}>
                {enviando ? "Enviando…" : exito ? "¡Enviado!" : "Confirmar pedido"}
              </button>
            </>
          )}
        </aside>
...
export default function ClienteView({ email, onLogout }) {
  ...
  useEffect(() => {
    const emojis = { pizza: "🍕", hamburguesa: "🍔", bebida: "🥤", pasta: "🍝", pollo: "🍗", arroz: "🍚" };
...
const mv = {
  ...
  cardEmoji: { background: "#f5f5f0", fontSize: 44, height: 90, display: "flex", alignItems: "center", justifyContent: "center" },
  ...
  exitoBox: { background: "#f0fff4", color: "#2d6a4f", borderRadius: 8, padding: "10px 14px", fontSize: 13, marginBottom: 12 },
};
const s = {
